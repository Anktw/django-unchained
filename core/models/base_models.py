...
class BaseModel(models.Model):
    ...
    created_by = models.CharField(max_length=20, unique=False, null=False, blank=False, default='system')
...
class UserManager(BaseUserManager):
    ...
    def with_perm(self, perm, is_active=True, include_superusers=True, backend=None,obj=None):
        if backend is None:
            ...
            backed = auth.load_backend(backend)
...
class BaseTenantModel(BaseModel):
    class Meta:
        abstract = True
        ordering = ['-updated_at']
    tenant = models.ForeignKey('Tenant', unique=False, blank=False, on_delete=models.CASCADE)
