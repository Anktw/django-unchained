...
def get_tenant_user_from_request_and_tenant(request, tenant):
    query = models.TenantUser.objects.filter(tenant_id=tenant.id, user_id=request.user.id)
    if not query.exists():
        raise ValidationError('Tenant user in the domain not found.')
    return query.get()

def get_tenant_user_from_request_and_tenant_and_tenant_user_id(request, tenant, **kwargs):
    ...
    query = models.TenantUser.objects.filter(pk=tenant_user_id, tenant_id=tenant.id, user_id=request.user.id)

    if not query.exists():
        raise ValidationError('Tenant user with the user id and the domain not found.')
...
def get_tenant_user_from_tenant_and_url_param(tenant, **kwargs):
    ...
    query = models.TenantUser.objects.filter(pk=tenant_user_id, tenant_id=tenant.id)
    if not query.exists():
        raise ValidationError('Tenant user in the domain not found.')
...
def tenant_user_data_api(wrapped_func):
    def tenant_user_data_api_impl(*args, **kwargs):
        ...
        tenant_user = get_tenant_user_from_request_and_tenant(request=args[1], tenant=tenant)
        _ = get_tenant_user_from_tenant_and_url_param(tenant=tenant, **kwargs)
        serializer = serializers.TenantUserSerializer(tenant_user)
        args += (Box(serializers.data),)
...
def self_tenant_user_data_api(wrapped_func):
    def self_tenant_user_data_api_impl(*args, **kwargs):
        ret = {'action': wrapped_func.__name__,}
        tenant = get_tenant_from_domain(**kwargs)
        tenant_user = get_tenant_user_from_request_and_tenant_and_tenant_user_id(request=args[1], tenant=tenant, **kwargs)
        serializer = serializers.TenantUserSerializer(tenant_user)
...
def _user_data_api(wrapped_func, allow_user_id_is_zero):
    ...
    def user_data_api_impl(*args, **kwargs):
        ...
        if user_id == 0:
            ...
            raise exceptions.OwnershipError('Given user_id and sender\'s user id not the same.')
        return wrapped_func(*args, **kwargs)
