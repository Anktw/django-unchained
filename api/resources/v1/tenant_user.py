...
from api.resources.decorators import tenant_user_api, tenant_api, self_tenant_user_data_api
...
class TenantUserListView(APIView):
    ...
    def post(self, request, tenant, domain):
        serializer = serializers.InvitedTenantUserSerializer(data=request.data, tenant=tenant, user=request.user, extra_request=dict(tenant_id=tenant.id))
    ...
    def filter_by_search_text(self, search, query):
        ...
        query = query.filter(Q(name_similarity__gt=constants.SEARCH_USER_SIMILARITY_THRESHOLD) | Q(user__email__icontains=search)).order_by('name_similarity')
        return query
    
    def filter_by_full_name(self, search, query):
        return query.annotate(username=Concat('user__first_name', Value(' '), 'user__last_name'))\
            .annotate(name_similarity=TrigramSimilarity('username', search))
    
    def filter_by_first_or_last_name(self, search, query):
        return query.annotate(fn_similarity=TrigramSimilarity('user__first_name', search))\
    ...
    def filter_by_email(self, search, query):
        return query.filter(user__email__icontains=search)
...
class TenantUserView(APIView):
    ...
    def get(self, request, tenant_user, domain, tenant_user_id):
        ...
        where &= Q(id=tenant_user_id) if tenant_user_id != 0 else (Q(user_id=request.user.id))
    ...
    def put(self, request, tenant_user, domain, tenant_user_id):
        ...
        serializer = serializers.TenantUserSerializer(tenant_user, data=request.data, user=tenant_user.user, partial=True)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    def delete(self, request, tenant_user, domain, tenant_user_id):
        admin_tenant_users = models.TenantUser.objects.filter(tenant_id=tenant_user.tenant.id, role_type=constants.TENANT_USER_ROLE_TYPE.ADMIN.value).exclude(pk=tenant_user_id)

        if admin_tenant_users.count() == 0:
            raise RuntimeError('If the user is deleted, no other admin user is in the tenant. Assign other user(s) to admin before deleting the user.')

        tenant_user = models.TenantUser.objects.get(pk=tenant_user_id, tenant_id=tenant_user.tenant.id)

        if (tenant_user_id != tenant_user.id) and (tenant_user.role_type != constants.TENANT_USER_ROLE_TYPE.ADMIN.value):
            raise RuntimeError('Can not delete account because user is not admin.')
