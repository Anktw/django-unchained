...
class TestUserSignUp:
    ...
    def test_signup_user(self, client, base_url, req, expected, email_verification_code):
        # Preprocess
        model = models.User
        n_data = model.objects.all().count()
    ...
    def test_signup_duplicate_user(self, client, base_url, email_verification_code):
        # Preprocess
        model = models.User
...
class TestUser:
    ...
    @pytest.mark.parametrize('req, expected',[
        (dict(my_seed=2, tgt_seed=1, data=dict(first_name='f002', last_name='l002', email='a002@a.com')),
        dict(status=403)),
        (dict(my_seed=2, tgt_seed=2, data=dict(first_name='f002', last_name='l002', email='a002@a.com')),
        dict(status=200)),
    ])
    ...
    @pytest.mark.parametrize('req, expected',[
        (dict(my_seed=1, tgt_seed=1, data=dict(password='wrong_password', new_password='new_password')),
        dict(status=400)),
        (dict(my_seed=1, tgt_seed=3, data=dict(password=default_password(), new_password='new_password')),
        dict(status=403)),
        (dict(my_seed=1, tgt_seed=1, data=dict(password=default_password(), new_password='new_password')),
        dict(status=200)),
    ])
    def test_update_user_password(self, client, bearer_token, base_url, req, expected):
        # Execution
        res = client.put(
...
class TestPasswordReset:
    ...
    @pytest.mark.parametrize('req, expected',[
        (dict(user_id=1, email_to_get_reset_code=email_from(seed=2), data=dict(email=email_from(seed=1), password='reset_password')),
        dict(status=400)),
        (dict(user_id=1, email_to_get_reset_code=email_from(seed=1), data=dict(email=email_from(seed=1), password='reset_password')),
        dict(status=200)),
    ])
    def test_reset_password(self, client, base_url, req, expected):
        ...
        # Common postprocess
        req_login = dict(data=dict(email=req['email_to_get_reset_code'], password=req['data']['password']))
